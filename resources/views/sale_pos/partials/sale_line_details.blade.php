<table class="table bg-gray">
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            @endif
            @if($sell->type == 'sales_order')
                <td><span class="display_currency" data-currency_symbol="false" data-is_quantity="true">{{ $sell_line->quantity - $sell_line->so_quantity_invoiced }}</span> @if(!empty($sell_line->sub_unit)) {{$sell_line->sub_unit}} @else {{$sell_line->product->unit}} @endif</td>
            @endif
            <td>
                ...
                <span class="display_currency" data-currency_symbol="false" data-is_quantity="true">{{ $sell_line->quantity }}</span> @if(!empty($sell_line->sub_unit)) {{$sell_line->sub_unit}} @else {{$unit_for}} @endif

                @if(!empty($sell_line->product->second_unit) && $sell_line->secondary_unit_quantity != 0)
                    <br>
                    <span class="display_currency" data-is_quantity="true" data-currency_symbol="false">{{ $sell_line->secondary_unit_quantity }}</span> {{$sell_line->product->second_unit}}
                @endif
            </td>
            @if(!empty($pos_settings['inline_service_staff']))
                <td>
