...
class ReportController extends Controller
{
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('report.partials.profit_loss_details', compact('data', 'super_admin'))->render();
        }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                // ->leftjoin('cheque_transactions', 'cheque_transactions.transaction_id', '=', 't.id')
                ->active()
                ->groupBy('contacts.id')
                ->select(
                    ...
                    // DB::raw("SUM(IF(cheque_transactions.transaction_payment_id = '0', (SELECT SUM(cheque_amount) FROM cheque_transactions WHERE cheque_transactions.transaction_id=t.id), 0)) as cheque_due"),
    ...
    public function getStockAdjustmentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $stock_adjustment_details = $query->select(
                ...
                DB::raw("SUM(IF(sub_type = 'increase', final_total, 0)) as total_increase"),
                DB::raw("SUM(IF(sub_type = 'decrease', final_total, 0)) as total_descrease")
            )->first();
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
        ...
        return view('report.sales_representative')
            ->with(compact('users', 'business_locations', 'customers', 'payment_types', 'pos_settings'));
    }
    ...
    public function getSalesRepresentativeTotalSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $sell_details = $this->transactionUtil->getSellTotals($business_id, $start_date, $end_date, $location_id, $created_by, $customer_id, $payment_status);
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('amount', function ($row) {
                    ...
                    // $cheque_due = $row->cheque_due;
                    // $amount = $amount + $cheque_due;
