...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createVariableProductVariations($product, $input_variations, $business_id = null, $return = null)
    {
        if (!is_object($product)) {
            $product = Product::find($product);
        }
        ...
        if (!empty($return)) {
            return Variation::orderBy('id', 'desc')->first()->id;
        }
    }
    ...
    //this is get product for stock adjustment for increase (this method $check_qty set as 'false')
    public function getDetailsFromVariationForIncrease($variation_id, $business_id, $location_id = null, $check_qty = false)
    {
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
            ->join('product_variations AS pv', 'variations.product_variation_id', '=', 'pv.id')
            ->leftjoin('variation_location_details AS vld', 'variations.id', '=', 'vld.variation_id')
    ...
    {
        $is_enable_product_variation = Business::first(); //automatic create product variation depend on purchase price
        ...
        foreach ($input_data as $data) {
            ...
            if ($is_enable_product_variation->product_variation_on_purchase == 1) {
                //check purchase price and sell price are equals
                $product_type = Product::find($data['product_id'])->type;
                $variation = Variation::where('id', $data['variation_id'])
                    ->where('product_id', $data['product_id'])
                    ->first();
                $variation_count = Variation::where('product_id', $data['product_id'])->count();
                $new_variation = true;
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                if ($is_enable_product_variation->product_variation_on_purchase == 1) {
                    if (!empty($variation) && $product_type == 'variable' && 3 > $variation_count) {
                        //  || ($variation->sell_price_inc_tax != $data['default_sell_price'])
                        if (($this->num_uf($variation->dpp_inc_tax, $currency_details) != $this->num_uf($data['purchase_price_inc_tax'], $currency_details))) {
                            $return_values = $this->createNewVariableProductVariations($product_type, $variation, $data, $new_variation, $new_variation_id);

                            $new_variation_id = $return_values[0];
                            $new_variation = $return_values[1];
                        }
                    }
                }
                ...
                if ($is_enable_product_variation->product_variation_on_purchase == 1 && !$new_variation) {
                    $purchase_line->variation_id = $new_variation_id;
                    $data['variation_id'] = $new_variation_id;
                }
    ...
    //    create new variation if price changes
    public function createNewVariableProductVariations($product_type, $variation, $data, $new_variation, $new_variation_id): array
    {
        //create new price batch if cost price or selling price changed
        $variation_template_id = $variation->product_variation->variation_template_id;
        $latest_variation_value_template_name = VariationValueTemplate::where('variation_template_id', $variation_template_id)->orderBy('id', 'desc')->first()->name;
        $temp = explode('-', $latest_variation_value_template_name);
        $value = isset($temp[1]) ? $temp[1] : 1;
        $latest_variation_value_template_name = $temp[0] . '-' . ++$value;
        $input_variations[0]['variation_template_id'] = $variation_template_id;
        $input_variations[0]['variations'][0] = [
            'sub_sku' => null,
            'variation_value_id' => '',
            'value' => $latest_variation_value_template_name,
            'default_purchase_price' => $data['purchase_price'],
            'dpp_inc_tax' => $data['purchase_price_inc_tax'],
            'profit_percent' => isset($data['profit_percent']) ? $data['profit_percent'] : 0,
            'default_sell_price' => isset($data['default_sell_price']) ? $data['default_sell_price'] : $data['purchase_price_inc_tax'],
            'sell_price_inc_tax' => isset($data['default_sell_price']) ? $data['default_sell_price'] : $data['purchase_price_inc_tax']
        ];

        $new_variation_id = $this->createVariableProductVariations($data['product_id'], $input_variations, null, 'Yes');
        $new_variation = false;

        return [$new_variation_id, $new_variation];
    }
