...
class TransactionUtil extends Util
{
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'cheque_paid_amount' => $uf_data ? $this->num_uf($payment['amount']) : $payment['amount'],
                    ...
                    //if payment method is cheque, save data in variables
                    if ($payment['method'] == 'cheque') {
                        $cheque_date = $uf_data ? $this->uf_date($payment['cheque_issued_date'], true) : $payment['cheque_issued_date'];
        ...
        if (!empty($payments_formatted)) {
            ...
            foreach ($account_transactions as $account_transaction) {
                ...
                if (!empty($payment)) {
                    ...
                    //if payment method is cheque, then update transaction payment table cheque id
                    if ($payment->method == 'cheque') {
                        $cheque_inputs['cheque_number'] = $payment->cheque_number;
                        $cheque_inputs['cheque_issued_date'] = $payment->paid_on;
                        $cheque_inputs['cheque_date'] = $cheque_date;
                        $cheque_inputs['cheque_amount'] = $payment->amount;
                        $cheque_inputs['cheque_status'] = 'paid';
                        $cheque_inputs['cheque_type'] = $cheque_type;
                        ...
                        TransactionPayment::where('id', $payment->id)
                            ->update([
                                'cheque_id' => $cheque->id,
                            ]);
    ...
    public function createOrUpdatePaymentLinesFroPop($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        $final_total = $this->num_uf($transaction->final_total);
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($payment['method'] != 'cheque') {
                        ...
                        $cheque_inputs['cheque_number'] = $payment['cheque_number'];
                        $cheque_inputs['cheque_issued_date'] = $paid_on;
                        $cheque_inputs['cheque_date'] = $uf_data ? $this->uf_date($payment['cheque_issued_date'], true) : $payment['cheque_issued_date'];
                        $cheque_inputs['cheque_amount'] = $payment_amount;
    ...
    //create payment amount for bulk return
    public function createPaymentLinesForBulkReturn($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($payment['method'] != 'cheque') {
                        ...
                        //create account transaction
                        $payment_data['transaction_type'] = 'sell_return';
                        $account_transactions[$c] = $payment_data;
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        unset($payment['cheque_issued_date']);
        unset($payment['cheque_type']);
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //Address
        $output['address'] = '';
        $output['landscape_address'] = $location_details->landmark;
        $temp = [];
        ...
        if ($il->show_alternate_number == 1 && !empty($location_details->alternate_number)) {
            if (empty($output['contact'])) {
                ...
                $output['location_contact'] .= $location_details->alternate_number;
            } else {
        ...
        if ($il->show_customer == 1) {
            ...
            if (!empty($customer->address_line_1)) {
                ...
                $customer_address_inline[] = ' ' . $customer->address_line_1;
            }
            if (!empty($customer->address_line_2)) {
                $customer_address[] =  '<br>' . $customer->address_line_2;
                $customer_address_inline[] =  ' ' . $customer->address_line_2;
            }
            if (!empty($customer->city)) {
                $customer_address[] = '<br>' . $customer->city;
                $customer_address_inline[] = ' ' . $customer->city;
            }
            if (!empty($customer->state)) {
                $customer_address[] = $customer->state;
                $customer_address_inline[] = ' ' . $customer->state;
            }
            if (!empty($customer->country)) {
                $customer_address[] = $customer->country;
                $customer_address_inline[] = ' ' . $customer->country;
            }
            ...
            $output['customer_address_inline'] = '';
            if (!empty($customer_address)) {
                $output['customer_info_address'] = implode(', ', $customer_address);
                $output['customer_address_inline'] = implode(', ', $customer_address_inline);
            }
        }
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            //Get payment details
            $output['payments'] = [];
            $output['is_cheque_payment'] = false;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array['landscape_sub_sku'] = !empty($variation->sub_sku) ? $variation->sub_sku : '';
            if ($il->show_image == 1) {
     ...
     * @return array
     */
    public function purchaseDetailsSellLines($lines, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_exc_tax' => $this->num_f($line->unit_price, false, $business_details),
                'price_exc_tax' => $line->quantity * $line->unit_price,
                'unit_price_before_discount' => $this->num_f($line->purchase_price_inc_tax, false, $business_details),
                'unit_price_before_discount_uf' => $line->purchase_price_inc_tax,
                //Fields for 4th column
                'line_total' => $this->num_f($line->purchase_price_inc_tax * $line->quantity, false, $business_details),
                'line_total_uf' => $line->unit_price_inc_tax * $line->quantity,
                'line_total_exc_tax' => $this->num_f($line->unit_price * $line->quantity, false, $business_details),
                'line_total_exc_tax_uf' => $line->unit_price * $line->quantity,
                'variation_id' => $variation->id
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = Transaction::where('business_id', $business_id)
            // ->leftjoin('cheque_transactions', 'cheque_transactions.transaction_id', '=', 'transactions.id')
            ->where('type', 'purchase')
            ->select(
                ...
                // DB::raw("SUM(IF(cheque_transactions.transaction_payment_id = '0', (SELECT SUM(cheque_amount) FROM cheque_transactions WHERE cheque_transactions.transaction_id=transactions.id), 0)) as cheque_due")
            );
        ...
        // $cheque_due = $purchase_details->cheque_due;
     ...
     * @return array
     */
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $customer_id = null, $payment_status = null)
    {
        ...
        $cheque_due = $sell_details->cheque_due;
     ...
     * @return void
     */
    public function multiplePayCheque($parent_payment, $type, $cheque_id, $selected_transactions)
    {
        ...
        if ($due_transactions->count()) {
            ...
            foreach ($due_transactions as $transaction) {
                if (in_array($transaction->id, $selected_transactions)) {
                    $transaction_before = $transaction->replicate();
                    //If sell check status is final
                    if ($transaction->type == 'sell' && $transaction->status != 'final') {
                        continue;
                    ...
                    if ($total_amount > 0) {
                        ...
                        $cheque_array = [
                            'cheque_id' => $cheque_id,
                            'transaction_id' => $transaction->id,
                            'contact_id' => $parent_payment->payment_for
                        ];

                        if ($due <= $total_amount) {
                            $array['amount'] = $due;
                            $cheque_array['cheque_amount'] = $due;

                            $tranaction_payments[] = $array;
                            $cheque_tranaction[] = $cheque_array;
                            ...
                            $array['amount'] = $total_amount;
                            $cheque_array['cheque_amount'] = $total_amount;
            ...
            //insert cheque transactions
            if (!empty($tranaction_payments)) {
                ChequeTransaction::insert($cheque_tranaction);
            }
        }
        return $total_amount;
    }
    ...
    public function getTransactionTotals(
        ...
        //bulk sell return
        $query->addSelect(
            DB::raw("SUM(IF(transactions.sub_type='bulk_sell_return', bulk_return_payment_amount, 0)) as total_bulk_refund"),
            DB::raw("SUM(IF(transactions.sub_type='bulk_sell_return', final_total, 0)) as total_bulk_replace")
        );
        ...
        //bulk sell return
        $output['total_bulk_refund'] =
            !empty($transaction_totals->total_bulk_refund) ?
            $transaction_totals->total_bulk_refund : 0;

        $output['total_bulk_replace'] =
            !empty($transaction_totals->total_bulk_replace) ?
            $transaction_totals->total_bulk_replace : 0;
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        //bulk sell return
        $data['total_bulk_return_refundable'] = $transaction_totals['total_bulk_refund'];
        $data['total_bulk_return'] = $transaction_totals['total_bulk_refund'] + $transaction_totals['total_bulk_replace'];
        ...
        $data['net_profit'] = $module_total + $gross_profit
            + ($data['total_sell_round_off'] + $data['total_recovered'] + $data['total_sell_shipping_charge'] + $data['total_purchase_discount'] + $data['total_sell_additional_expense'] + $data['total_sell_return_discount']
                ...
                + $data['total_bulk_return_refundable']);
    ...
    public function payContactForBulkReturn($bulk_transaction_id, $contact_id, $payment, $format_data = true)
    {

        foreach ($payment as $inputs) {
            # code...
        }
        ...
        //     'amount', 'method', 'note', 'card_number', 'card_holder_name',
        //     'card_transaction_number', 'card_type', 'card_month', 'card_year', 'card_security',
        //     'cheque_number', 'bank_account_number', 'due_amount'
        // ]);
        ...
        if ($format_data) {
            ...
            if ($this->num_uf($inputs['amount']) > $this->num_uf($inputs['due_amount'])) {

                $inputs['amount'] = $this->num_uf($inputs['due_amount']);
            }
        }
        ...
        $inputs['bulk_return_transaction_id'] = $bulk_transaction_id;

        $contact = Contact::where('business_id', $business_id)
            ->findOrFail($contact_id);

        $due_payment_type = $inputs['due_payment_type'];
        if (empty($due_payment_type)) {
            $due_payment_type = $contact->type == 'supplier' ? 'purchase' : 'sell';
        }
        ...
        //Upload documents if added
        // $inputs['document'] = $this->uploadFile($inputs, 'document', 'documents');

        unset($inputs['due_payment_type']);
        unset($inputs['due_amount']);
        unset($inputs['replace_amount']);
        unset($inputs['cheque_issued_date']);
        unset($inputs['cheque_type']);
        unset($inputs['transaction_no_1']);
        unset($inputs['transaction_no_2']);
        unset($inputs['transaction_no_3']);
        unset($inputs['transaction_no_4']);
        unset($inputs['transaction_no_5']);
        unset($inputs['transaction_no_6']);
        unset($inputs['transaction_no_7']);
    ...
    public function addBulkSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        $discount = [
            'discount_type' => $input['discount_type'] ?? 'fixed',
            'discount_amount' => $input['discount_amount'] ?? 0
        ...
        if (empty($sell_return)) {
            ...
            // $sell_return_data['customer_group_id'] = $sell->customer_group_id;
            $sell_return_data['type'] = 'sell_return';
            $sell_return_data['sub_type'] = 'bulk_return';
            $sell_return_data['status'] = 'final';
            $sell_return_data['created_by'] = $user_id;
            // $sell_return_data['return_parent_id'] = $sell->id;
            $sell_return = Transaction::create($sell_return_data);
        ...
        foreach ($input['products'] as $product) {
            $line['transaction_id'] = $sell_return->id;
            $line['product_id'] = $product['product_id'];
            $line['variation_id'] = $product['variation_id'];
            $line['quantity'] = 0;
            $line['unit_price_before_discount'] = $product['unit_price_inc_tax'];
            $line['unit_price'] = $product['unit_price_inc_tax'];
            $line['unit_price_inc_tax'] = $product['unit_price_inc_tax'];
            $line['quantity_returned'] = $product['quantity'];
        }

        $sell_lines = TransactionSellLine::create($line);

        //Update quantity returned in sell line
        if ($input['return_type'] == 'replace') {
